...
from successive_refinement.pix2pix_models.pix2pix_model_vq import *
...
parser.add_argument('--frozen_generate', action='store_true', help='should the generator be frozen')
parser.add_argument('--frozen_checkpoint', type=str, default='.', help='frozen checkpoint')
# training parameters
...
parser.add_argument('--lr_generate', type=float, default=0.0002, help='initial learning rate for adam')
parser.add_argument('--lr_refine', type=float, default=0.0002, help='initial learning rate for adam')
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Now threshold the noisy sample
        # thresholded_sample = SingleCrossingThreshold()(noisy_sample)
...
def main():
    ...
    args.display_freq = 20 * batch_size
    args.print_freq = 10 * batch_size
    ...
    # If the generator is to be kept frozen, set the learning rate very low
    # and load the frozen checkpoint
    if args.frozen_generate:
        cp = torch.load(args.frozen_checkpoint)
        model.generate.load_state_dict(cp['netG_state_dict'])
        del cp
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for i, sample in enumerate(dataloader):
            ...
            if epoch_iter % args.display_freq == 0:
                ...
                detached_refined = refined.detach().cpu()
                visuals = OrderedDict()
                ...
                visuals['refined_recon_image' + str(i*batch_size + j)] = detached_refined[j, ...]

                writer.add_image(f'{epoch}/{i}/quantized', visuals['quantized' + str(i*batch_size + j)], i)
                ...
                writer.add_image(f'{epoch}/{i}/refined_recon_image', visuals['refined_recon_image' + str(i*batch_size + j)], i)
