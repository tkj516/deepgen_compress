...
from models.pix2pix_model_crossing_undoing import *
...
# basic parameters
parser.add_argument('--dataroot', help='path to images (should have subfolders trainA, trainB, valA, valB, etc)', default='./CelebA/celeba/img_align_celeba')
...
parser.add_argument('--print_freq', type=int, default=1600, help='frequency of showing training results on console')
...
class ImageDataset(Dataset):
    ...
    def __init__(self, 
                root_dir='./CelebA/celeba/img_align_celeba',
                phase='test', 
                transform=None,
                noise_file='.'):
        ...
        self.noise = torch.load(args.noise_file)
    ...
    def __getitem__(self, idx):
        ...
        compressed_sample = SingleCrossingThreshold()(noisy_sample)

        return {'compressed_sample': compressed_sample, 'sample': sample}
...
def main():
    ...
    for i, sample in enumerate(dataloader):
        ...
        if total_iters % args.display_freq == 0:
            ...
            visuals['thresholded' + str(i*batch_size + j)] = sample['compressed_sample'][j, ...]
            visuals['original' + str(i*batch_size + j)] = sample['sample'][j, ...]
            visuals['output_recon_image' + str(i*batch_size + j)] = detached_out[j, ...]

            writer.add_image(f'{i}/thresholded', visuals['thresholded' + str(i*batch_size + j)], i)
