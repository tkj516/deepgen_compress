...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[], threshold_levels=[-0.25, -0.10, 0.0, 0.10, 0.25, 0.50, 0.75]):
    """Create a generator
    ...
    if netG == 'resnet_9blocks':
        ...
        net = MTGAN(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9, threshold_levels=threshold_levels)
    else:
...
class MTGAN(nn.Module):
    ...
    def forward(self, x):
        ...
        x_t = []
        for i in range(len(self.threshold_levels)):
            x_t.append(self.thresholders[i](x))
        x_t = torch.cat(x_t, dim=1).to(x.device)
