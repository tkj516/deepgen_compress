...
parser.add_argument('--gan_arch', type=str, default='convmvqsrgan', help='choose the type of decoder GAN architecture')
...
def main():
    ...
    args.input_nc = sum([int(i) for i in args.num_kernels.split(',')])
    num_vq = args.input_nc + 1  # Plus one because of original image
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for i, sample in enumerate(dataloader):
            ...
            filtered_images = model.filtered_images
            ...
            if epoch_iter % args.display_freq == 0:
                ...
                detached_filtered_images = filtered_images.detach().cpu()
                visuals = OrderedDict()
                ...
                for k in range(num_vq):  
                    ...
                    visuals['filtered_image (VQ' + str(k) + ') ' + str(i*batch_size + j)] = detached_filtered_images[j, k, ...].unsqueeze(0)
                ...
                for k in range(num_vq):
                    ...
                    writer.add_image(f'{epoch}/{i}/filtered_image (VQ{k})', visuals['filtered_image (VQ' + str(k) + ') ' + str(i*batch_size + j)], i)
