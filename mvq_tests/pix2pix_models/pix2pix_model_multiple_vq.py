...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # Used for conv VQ architectures
        self.kernel_sizes = [int(i) for i in opt.kernel_sizes.split(',')]
        self.num_kernels = [int(i) for i in opt.num_kernels.split(',')]

        self.gan_arch = opt.gan_arch
        ...
        if self.isTrain:
            self.netD = networks.define_D(input_nc=opt.output_nc, 
                                          ndf=opt.ndf, 
                                          init_type=opt.init_type,
                                          init_gain=opt.init_gain, 
                                          gpu_ids=self.gpu_ids)
    ...
    def forward(self):
        ...
        if self.gan_arch == 'convmvqsrgan':
            self.commitment_loss, self.encodings, self.fake_B, self.filtered_images = self.netG(self.gt)
        elif self.gan_arch == 'patchmvqsrgan':
            self.commitment_loss, self.encodings, self.fake_B, self.shifted_images = self.netG(self.gt)
        else:
            self.commitment_loss, self.encodings, self.fake_B = self.netG(self.gt)  # G(A)
