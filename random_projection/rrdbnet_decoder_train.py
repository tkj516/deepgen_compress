...
from random_projection.pix2pix_models.pix2pix_model_projection import *
...
parser.add_argument('--gan_arch', type=str, default='rrdbnet', help='choose the type of decoder GAN architecture')
...
def main():
    ...
    print(f"[Starting slope for slope annealing is {model.netG.module.m}]")
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for i, sample in enumerate(dataloader):
            ...
            encoding = torch.matmul(model.ldpc_mat, sample['sample'].reshape(b, w*h, c).to(model.device))
            encoding = (encoding % 2).reshape(b, c, h//4, w//4)
            ...
            if epoch_iter % args.display_freq == 0:
                ...
                visuals['encoded ' + str(i*batch_size + j)] = detached_encoding[j, ...]
