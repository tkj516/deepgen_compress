...
class StyleGAN2Generator(nn.Module):
    ...
    def __init__(self,
        ...
        self.style_conv1 = StyleConv(
            ...
            sample_mode=None,
            resample_kernel=resample_kernel)
        self.to_grayscale1 = ToGrayscale(
            channels['4'],
            num_style_feat,
            upsample=False,
            resample_kernel=resample_kernel)
        ...
        # TODO: Sigmoid for DBN and slope for annealing
    ...
    def forward(self,
        ...
        skip = self.to_grayscale1(out, latent[:, 1])

        i = 1
        for conv1, conv2, noise1, noise2, to_grayscale in zip(
                self.style_convs[::2], self.style_convs[1::2], noise[1::2],
                noise[2::2], self.to_grayscales):
            out = conv1(out, latent[:, i], noise=noise1)
            out = conv2(out, latent[:, i + 1], noise=noise2)
            skip = to_grayscale(out, latent[:, i + 2], skip)
            i += 2

        image = skip

        # TODO: Add deterministic binary neuron with slope annealing
        binary_image = self.sigmoid(self.m*image) + (torch.heaviside(self.sigmoid(image)-0.5, torch.tensor([0.0]).to(image.device)) - self.sigmoid(self.m*image)).detach()

        if return_latents:
            return binary_image, latent
        else:
...
class StyleGAN2Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        # TODO: Make the change to 1 channel
        conv_body = [
            ConvLayer(1, channels[f'{out_size}'], 1, bias=True, activate=True)
        ]
