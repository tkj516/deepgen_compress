...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, init_type=opt.init_type, init_gain=opt.init_gain, gpu_ids=self.gpu_ids)

        # define relativistic discriminator
        if self.isTrain:
            self.netD = networks.define_D(opt.input_nc, ndf=opt.ndf, init_type=opt.init_type, init_gain=opt.init_gain, gpu_ids=self.gpu_ids)
    ...
    def backward_G(self):
        ...
        b, c, w, h = self.fake_B.shape
        pred_projection = torch.matmul(self.ldpc_mat, self.fake_B.reshape(b, w*h, c))
        true_projection = torch.matmul(self.ldpc_mat, self.gt.reshape(b, w*h, c))
        self.loss_G_diff = self.criterionMSE(pred_projection, true_projection)
        # combine loss and calculate gradients
        self.loss_G = self.loss_G_GAN + self.loss_G_L1 + self.loss_G_diff
