...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(netG='ffgan', gpu_ids=self.gpu_ids)

        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = networks.define_D(netD='ffgan', gpu_ids=self.gpu_ids)
    ...
    def set_input(self, input, encoding):
        ...
        self.real_B = input['sample'].reshape(b, h*w).to(self.device)
        self.real_A = encoding.to(self.device)

    def forward(self):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        self.fake_B = self.netG(self.real_A)  # G(A)
        # self.fake_B = self.fake_B.reshape(self.fake_B.shape[0], -1)
    ...
    def backward_G(self):
        ...
        b, l = self.fake_B.shape
        pred_projection = torch.matmul(self.ldpc_mat, self.fake_B.reshape(b, l, 1))
        true_projection = torch.matmul(self.ldpc_mat, self.real_B.reshape(b, l, 1))
