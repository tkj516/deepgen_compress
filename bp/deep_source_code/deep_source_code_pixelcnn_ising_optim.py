...
class SourceCodeBP():
    ...
    def decode_step_pixelcnnpp(self):
        ...
        probs_0 = torch.where(torch.tanh(self.train_in) <= 0, self.probs, 1-self.probs)
        probs_1 = torch.where(torch.tanh(self.train_in) > 0, self.probs, 1-self.probs)
        self.probs = torch.cat([probs_0, probs_1], 1)
