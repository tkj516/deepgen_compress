...
from code_bp_numpy import CodeBP
from grid_bp_numpy import GridBP
...
class SourceCodeBP():
    ...
    def __init__(self,
        ...
        self.ps = np.tile(np.array([1-p, p]), (h*w, 1))
    ...
    def doping(self):
        ...
        self.ps[indices, 0], self.ps[indices, 1] = (self.samp[indices, 0] == 0).astype(float), (self.samp[indices, 0] == 1).astype(float)
    ...
    def generate_sample(self):
        ...
        self.samp = self.sampler.samp.reshape(-1, 1)
    ...
    def decode(self, num_iter=100):

        B_old = float('nan') * np.ones((self.h, self.w))
        for i in tqdm(range(num_iter)):
            ...
            self.B /= np.sum(self.B, -1)[..., np.newaxis]

            if np.sum(np.abs(self.B[..., 1] - B_old)) < 0.5:
                break
            B_old = self.B[..., 1]
            errs = np.sum(np.abs((self.B[..., 1] > 0.5) - self.samp.reshape(self.h, self.w)))
            print(errs)
...
def test_source_code_bp():
    ...
    source_code_bp = SourceCodeBP(H, h=128, w=128)

    print("[Generating the sample ...]")
    source_code_bp.generate_sample()
    # source_code_bp.samp = loadmat('S.mat')['Sb'].astype(float).reshape(-1, 1)
    ...
    print("[Decoding ...]")
    source_code_bp.decode()

    fig, ax = plt.subplots(2, 1)
    ax[0].imshow(source_code_bp.samp.reshape(128, 128))
    ax[1].imshow(source_code_bp.B[:,:,1])
    plt.show()
