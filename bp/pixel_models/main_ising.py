...
# data params
parser.add_argument('--dataset', choices=['mnist', 'colored-mnist', 'cifar10', 'ising'])
parser.add_argument('--n_cond_classes', type=int, help='Number of classes for class conditional model.')
parser.add_argument('--n_bits', type=int, default=1, help='Number of bits of input data.')
parser.add_argument('--image_dims', type=int, nargs='+', default=(3,28,28), help='Dimensions of the input data.')
parser.add_argument('--data_path', default='/fs/data/tejasj/mrf/mrf_128', help='Location of datasets.')
# training param
...
def fetch_dataloaders(args):
    ...
    if args.dataset == 'ising':
        args.image_dims = (1,128,128)
        train_dataset = MRFDataset(args.data_path, phase='train', transform=transform)
        valid_dataset = MRFDataset(args.data_path, phase='test', transform=transform)
...
if __name__ == '__main__':
    ...
    if args.model=='pixelcnn':
        ...
        import pixelcnnpp
        model = MyDataParallel(pixelcnnpp.PixelCNNpp(args.image_dims, args.n_channels, args.n_res_layers, args.n_logistic_mix,
                                      args.n_cond_classes))
