...
class SourceCodeBP():
    ...
    def decode_step(self):
        ...
        self.M_from_grid = torch.masked_fill(self.M_from_grid, torch.tensor(self.mask).to(device) < 1, 0.5)
    ...
    def decode(self, num_iter=1):
        ...
        for i in range(num_iter):
            ...
            ax[0, 1].imshow((self.npot.cpu()[..., 1] > 0.5).float().numpy(), vmin=0, vmax=1)
            ax[0, 1].set_title("Doping samples")
            ax[1, 0].imshow((self.B.cpu()[..., 1] > 0.5).float().numpy(), vmin=0, vmax=1)
            ax[1, 0].set_title("Reconstructed Image")
            ax[1, 1].imshow(self.M_from_grid.detach().cpu()[..., 1].numpy(), vmin=0, vmax=1)
            ax[1, 1].set_title("M from grid")
            ax[2, 0].imshow(self.M_to_grid.detach().cpu()[..., 1].numpy(), vmin=0, vmax=1)
