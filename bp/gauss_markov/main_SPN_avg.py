...
in_size = (1, 32, 32)
dataset = GaussMarkovDataset(phase='test')
    ...
    for b in [3, 5, 7, 8]:
        for w in [0.1, 0.2, 0.4, 0.6, 0.8]:
            ...
            while min_rate < max_rate:
                ...
                # Decode
                s_hat = decode_SPN(x, dope, H, Q, Q0, b, max_iter, convg_epsilon, checkpoint='/fs/data/tejasj/Masters_Thesis/deepgen_compress/bp/spn_code_decoding/dgcspn/gauss-markov/generative/model_2022-03-24_13:20:25.pt')
                ...
                # Check for NaN and automatically increase lower bound
                if np.isnan(sqnr):
...
filepath = os.path.join('demo_gray_lossy', 'gauss-markov-spn', 'results.json')
os.makedirs(os.path.join('demo_gray_lossy', 'gauss-markov-spn'), exist_ok=True)
