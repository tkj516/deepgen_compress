...
class CodeBP(nn.Module):
    ...
    def forward(self, ps, x, Min):
        ...
        grid = torch.LongTensor([[i]*3 for i in range(self.N)])
        factor_neighbors = torch.LongTensor(self.neighbors_of_variable)
        # Update messages first using external beliefs
        self.Hsx[factor_neighbors, grid] = log_Min.unsqueeze(-1) + log_ps.unsqueeze(-1)
        ...
        self.Hsx[factor_neighbors[check[:,0], :], grid[check[:,0], :]] += ll_sum[check[:,0]] - \
                                                        self.Hxs[factor_neighbors[check[:,0], :], grid[check[:,0], :]]
        # One sure neighbor
        ...
        if check1.shape[0] > 0:
            ...
            self.Hsx[factor_neighbors[check1[:,0], check1[:,1]], grid[check1[:,0], check1[:,1]]] += ll_sum[check1[:,0], 0]
        if check2.shape[0] > 0:
            # Other neighbors should be set to the sure neighbor's value
            self.Hsx[factor_neighbors[check2[:,0], check2[:,1]], grid[check2[:,0], check2[:,1]]] = sure_sum[check2[:,0], 0]
        ...
        if check1.shape[0] > 0:
            ...
            self.Hsx[factor_neighbors[check1[:,0], :], grid[check1[:,0], :]] = 0
            self.Hsx[factor_neighbors[check2[:,0], :], grid[check2[:,0], :]] += sure_sum[check2]

        #####################################################################################################
        # Factor to node messages
        #####################################################################################################
        
        grid = torch.LongTensor([[i]*48 for i in range(self.K)])
        variable_neighbors = torch.LongTensor(self.neighbors_of_factor)

        ll_diff_prod = 1 - 2*x
        # Calculate all the log-likelihood terms that will be multiplied at each factor
        ll_diff = torch.tanh(self.Hsx[grid, variable_neighbors])
        ...
        if check.shape[0] > 0:
            self.Hxs[grid[check[:,0], :], variable_neighbors[check[:,0], :]] = 0
        # One null neighbor
        ...
        if check1.shape[0] > 0:
            self.Hxs[grid[check1[:,0], check1[:,1]], variable_neighbors[check1[:,0], check1[:,1]]] = 0
        if check2.shape[0] > 0:
            self.Hxs[grid[check2[:,0], check2[:,1]], variable_neighbors[check2[:,0], check2[:,1]]] = \
                                                                torch.atanh(ll_diff_prod[check2[:,0], 0])
        # Zero null neighbors
        check = torch.nonzero((null_neighbors == 0).float())
        if check.shape[0] > 0:
            self.Hxs[grid[check[:,0], :], variable_neighbors[check[:,0], :]] = torch.atanh(ll_diff_prod[check[:,0]] / ll_diff[check[:,0], :])
        ...
        M_out_diff = torch.tanh(torch.sum(self.Hxs[factor_neighbors, grid], -1, keepdim=True))
        ...
        if nan_check.shape[0] > 0:
            M_out_diff[nan_check[:,0], 0] = \
                torch.tanh(torch.sum(self.Hxs[factor_neighbors, grid].masked_fill(nan_check, 0), -1, keepdim=True))[nan_check[:,0],0]
        # Convert likelihoods back to probability
        self.M_out = 0.5 + torch.cat([M_out_diff, -M_out_diff], -1)/2
