...
class IsingGenerator(nn.Module):
    ...
    def __init__(self,
        ...
        self.body = nn.Sequential(nn.ConvTranspose2d(1, 128, 5, stride=1, padding=2),
                                  ...
                                  nn.BatchNorm2d(64),
                                  nn.LeakyReLU(),
                                  nn.ConvTranspose2d(64, 2, 5, stride=2, padding=2, output_padding=1),
                                  )
...
class IsingDiscriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.embedder = nn.Sequential(nn.Conv2d(1, 16, 5, stride=2, padding=2),
                                      nn.LeakyReLU(),
                                      nn.Dropout2d(0.3),
        ...
        self.judgement = nn.Sequential(nn.Conv2d(33, 2, 3, stride=1, padding=1),
                                      nn.LeakyReLU(),
                                      nn.Flatten(),
                                      nn.Linear(2048, 1024),
                                      nn.LeakyReLU(),
    ...
    def forward(self, x, encoding):
        ...
        out = self.embedder(x)
        out = torch.cat([out, encoding.reshape(-1, 1, 32, 32)], dim=1).to(x.device)
        out = self.judgement(out)
